...
const DashboardLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <div className="flex h-screen bg-slate-50 text-slate-900 font-sans">
      {/* Sidebar - Reference the lavender color from screenshot with gradient */}
      <aside className="w-72 bg-gradient-to-b from-[#F0EFF8] to-[#E3E1F0] border-r border-[#D9D6E9] flex flex-col">
        {/* Sidebar Header */}
        <div className="p-5 flex items-center gap-2 border-b border-[#D9D6E9]/50 bg-white/20">
          <div className="bg-indigo-600 p-2 rounded-xl shadow-sm">
            <Sparkles className="text-white w-4 h-4" />
          </div>
          <div>
            <span className="font-bold text-base tracking-wide text-slate-800 block">智能 AI 中台</span>
          </div>
        </div>
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8">
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-indigo-50 text-indigo-700 flex items-center justify-center font-bold text-xs shadow-sm border border-indigo-100">
                张
              </div>
              <div className="flex flex-col text-left">
